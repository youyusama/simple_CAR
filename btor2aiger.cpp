...
// The source code is slightly modified so that the converted aiger has a fixed state order
// Yechuan xia
    ...
    while ((l = btor2parser_iter_next(&it))) {
        ...
        case BTOR2_TAG_state:
            ...
            if (l->tag == BTOR2_TAG_input)
                ...
                model.states.push_back(std::pair<int64_t, BoolectorNode *>(l->id, node));
            break;
